...
var commands = []commandSpec{
	...
	{name: "completion", desc: "Generate shell completion (bash/zsh/fish/powershell)"},
	{name: "install-git", desc: "Install Git via winget on Windows"},
...
func runCompletion(args []string) error {
	if len(args) < 1 {
		ui.Error("usage: git-user completion <bash|zsh|fish|powershell>")
		fmt.Println()
		...
		fmt.Println("    git-user completion powershell | Out-String | Invoke-Expression")
	...
	default:
		...
		ui.Info("Supported shells: bash, zsh, fish, powershell")
...
func fishCompletion() string {
	...
	out += "complete -c git-user -f -n \"__fish_seen_subcommand_from completion\" -a \"bash zsh fish powershell\"\n"
	out += "complete -c gu -f -n \"__fish_seen_subcommand_from completion\" -a \"bash zsh fish powershell\"\n\n"
...
func powershellCompletion() string {
	var cmdEntries []string
	for _, c := range commands {
		cmdEntries = append(cmdEntries, fmt.Sprintf("            @{ Name = '%s'; Desc = '%s' }", c.name, strings.ReplaceAll(c.desc, "'", "''")))
	}
	cmdsBlock := strings.Join(cmdEntries, "\n")

	var idCmds []string
	for _, id := range commandsThatTakeIDs() {
		idCmds = append(idCmds, fmt.Sprintf("'%s'", id))
	}
	idCmdsBlock := strings.Join(idCmds, ", ")

	return fmt.Sprintf(`# PowerShell completion for git-user and gu
# Generated by: git-user completion powershell

Register-ArgumentCompleter -Native -CommandName @('git-user', 'gu') -ScriptBlock {
    param($wordToComplete, $commandAst, $cursorPosition)

    $elements = $commandAst.CommandElements
    $count = $elements.Count

    if ($count -le 1 -or ($count -eq 2 -and -not $wordToComplete)) {
        $cmds = @(
%s
        )
        $cmds | Where-Object { $_.Name -like "$wordToComplete*" } | ForEach-Object {
            [System.Management.Automation.CompletionResult]::new($_.Name, $_.Name, 'Command', $_.Desc)
        }
        return
    }

    $subcommand = $elements[1].Extent.Text.ToLower()

    $idCommands = @(%s)
    if ($idCommands -contains $subcommand) {
        $identities = @()
        if (Get-Command git-user -ErrorAction SilentlyContinue) {
            $rawList = & git-user list --plain 2>$null
            if ($rawList) {
        ...
        $identities | Where-Object { $_ -like "$wordToComplete*" } | ForEach-Object {
            [System.Management.Automation.CompletionResult]::new($_, $_, 'ParameterValue', "Identity: $_")
        }
        return
    }
    ...
    if ($wordToComplete -like "-*") {
        ...
        if ($subcommand -eq "switch" -or $subcommand -eq "sw") {
            $flags += @{ Name = "--session"; Desc = "Switch session environment only" }
            $flags += @{ Name = "--local"; Desc = "Switch for current repository only" }
            $flags += @{ Name = "--global"; Desc = "Switch global git config" }
            $flags += @{ Name = "-c"; Desc = "Create and switch" }
        }
        if ($subcommand -eq "env") {
            $flags += @{ Name = "--unset"; Desc = "Unset session environment" }
            $flags += @{ Name = "--powershell"; Desc = "Output PowerShell commands" }
            $flags += @{ Name = "--cmd"; Desc = "Output CMD batch commands" }
            $flags += @{ Name = "--bash"; Desc = "Output POSIX shell commands" }
        }
