...
// TestOpAttachKeyGenerateNeverReusesStaleFile guards against the exact bug
// reported: renaming or deleting an identity leaves its git_<name> key file
// behind, and creating a new identity with that same name used to silently
// attach to that stale leftover instead of generating a fresh key. opBind's
// "generate" path (opAttachKey with a fixed default path, keyPath == "")
// must now refuse instead of reusing it.
func TestOpAttachKeyGenerateNeverReusesStaleFile(t *testing.T) {
	withTempConfig(t)
	...
	// A stale leftover key file — not generated by this test run, standing
	// in for whatever a previously renamed/deleted "dev" identity left behind.
	if err := os.WriteFile(keyPath, []byte("stale-leftover-key"), 0600); err != nil {
		t.Fatal(err)
	}

	store, _ := config.Load()
	_, err = opAttachKey(store, "dev", "dev@example.com", "register", "generate", "", "", false)
	if err == nil {
		t.Fatal("expected opAttachKey to refuse generating over an existing file, got nil error")
	}

	// The stale file must be untouched — proving nothing silently adopted it.
	content, readErr := os.ReadFile(keyPath)
	if readErr != nil || string(content) != "stale-leftover-key" {
		t.Errorf("expected the stale file to be left untouched, got content %q (err: %v)", content, readErr)
	}
}

// TestOpAttachKeyGenerateUsesProvidedKeyPath verifies that when the TUI's
// key-filename step has already resolved a (non-default) path, opAttachKey
// generates the key there instead of silently falling back to git_<name>.
func TestOpAttachKeyGenerateUsesProvidedKeyPath(t *testing.T) {
	if _, err := exec.LookPath("ssh-keygen"); err != nil {
		t.Skip("ssh-keygen not available")
	}
	withTempConfig(t)

	customPath, err := config.SSHKeyPathForFilename("my_custom_key")
	if err != nil {
		t.Fatal(err)
	}

	store, _ := config.Load()
	res, err := opAttachKey(store, "dev", "dev@example.com", "register", "generate", "", customPath, false)
	if err != nil {
		t.Fatalf("opAttachKey failed: %v", err)
	}
	if !strings.Contains(res.detail, "my_custom_key") {
		t.Errorf("expected report to mention the custom key path, got:\n%s", res.detail)
	}
	u := store.FindUser("dev")
	if u == nil || u.SSHKey != customPath {
		t.Fatalf("expected identity bound to %q, got %+v", customPath, u)
	}
	if _, err := os.Stat(customPath); err != nil {
		t.Errorf("expected key file to exist at %s: %v", customPath, err)
	}
}
...
// not have rekey silently regenerate an unrelated git_<name> file while
// leaving the real, actively-used key completely untouched.
func TestOpRekeyRotatesActuallyBoundKey(t *testing.T) {
	if _, err := exec.LookPath("ssh-keygen"); err != nil {
		t.Skip("ssh-keygen not available")
	}
	withTempConfig(t)
	...
	if err := os.MkdirAll(filepath.Dir(customPath), 0700); err != nil {
		t.Fatal(err)
	}
	if err := exec.Command("ssh-keygen", "-t", "ed25519", "-C", "dev@example.com", "-f", customPath, "-N", "").Run(); err != nil {
		t.Fatalf("generating custom key: %v", err)
	}
	origPub, err := os.ReadFile(customPath + ".pub")
	if err != nil {
		t.Fatal(err)
	}
	...
	store, _ := config.Load()
	_ = store.AddUser("dev", "dev@example.com")
	_ = store.BindSSHKey("dev", customPath)
	_ = config.Save(store)

	res, err := opRekey(store, "dev", "", "")
	if err != nil {
		t.Fatalf("opRekey failed: %v", err)
	}
	if !strings.Contains(res.detail, "rotated successfully") {
		t.Errorf("expected a success report, got:\n%s", res.detail)
	}

	u := store.FindUser("dev")
	if u == nil || u.SSHKey != customPath {
		t.Fatalf("expected identity to remain bound to %q, got %+v", customPath, u)
	}
	if _, err := os.Stat(customPath + ".backup"); err != nil {
		t.Errorf("expected the actually-bound key to be backed up at %s.backup: %v", customPath, err)
	}
	newPub, err := os.ReadFile(customPath + ".pub")
	if err != nil {
		t.Fatalf("expected a freshly generated public key at %s.pub: %v", customPath, err)
	}
	if string(newPub) == string(origPub) {
		t.Error("expected the public key to change after rotation")
	}

	// The unrelated default path must never have been touched.
...
func TestOpRekeyRefusesToOverwriteUnrelatedFile(t *testing.T) {
	...
	otherPath, err := config.SSHKeyPathForFilename("someone_elses_key")
	if err != nil {
		t.Fatal(err)
	}
	...
	_, err = opRekey(store, "dev", otherPath, "")
	if err == nil {
		t.Fatal("expected opRekey to refuse rotating into an already-occupied filename")
	}
	content, readErr := os.ReadFile(otherPath)
	if readErr != nil || string(content) != "unrelated" {
		t.Errorf("expected the unrelated file to be left untouched, got content %q (err: %v)", content, readErr)
	}
}
