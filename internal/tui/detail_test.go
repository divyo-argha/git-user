...
func TestDetail(t *testing.T) {
	...
	msg := cmd()
	if _, ok := msg.(ScreenPopMsg); !ok {
		t.Errorf("Expected ScreenPopMsg on Esc")
	}
	...
	msg = cmd()
	if actionMsg, ok := msg.(ActionResultMsg); ok {
		if actionMsg.Kind != "switch" {
			t.Errorf("Expected switch action, got %s", actionMsg.Kind)
		}
	} else {
		t.Errorf("Expected ActionResultMsg on Enter, got %T", msg)
	}
}
